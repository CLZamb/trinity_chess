...
string KeyboardInputPlayer::get_player_string_move(PlayerPosition &player_pos) {
  completed = false;
  has_been_selected = false;

  string next_move{""};
  size_t next_pos = player_pos.get_to_position();
  ...
  while (!completed) {
    switch (InputKeys::Key key = m_k_input.read_key(); key) {
      ...
      case InputKeys::ENTER:
        next_move += select_position(player_pos, next_pos);
