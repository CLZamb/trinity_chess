...
void InfoPane::set_content_to_block_recursively(shared_ptr<Section>& section,
  ...
  if (msg.size() < kRowMaxLen) {
    section->set_content_at_index(format_line(msg), current_row);
    section->set_content_at_index(m_row_divider, section->size() - 1);
    return;
  }

  string row_content = msg.substr(0, kRowMaxLen);
  section->set_content_at_index(format_line(row_content), current_row);
