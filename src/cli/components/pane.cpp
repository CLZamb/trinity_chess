...
size_t Pane::size() { return this->m_pane_drawing.size(); }
...
void Pane::set_content_at_section(const string &key,
                                  std::initializer_list<string> lst) {
...
bool Pane::has_block_space_for_content(const std::shared_ptr<Section> &block,
...
void Pane::set_content_to_block_recursively(std::shared_ptr<Section> &section,
  ...
  if (msg.size() < kRowMaxWidth) {
    ...
    section->set_drawing_at_index(PaneBorders::krow_divider,
                                  section->size() - 1);
    return;
  }

  section->set_drawing_at_index(format_line(msg.substr(0, kRowMaxWidth)),
                                current_row);
