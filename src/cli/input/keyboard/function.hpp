...
class bind {
public:
    bind(Function func, Object obj, Args... args)
        : m_func(func), m_obj(obj), m_args(args...) {}

    template <typename... ExtraArgs>
    auto operator()(ExtraArgs&&... extra_args) {
        return call(std::index_sequence_for<ExtraArgs...>(), 
                    std::forward<ExtraArgs>(extra_args)...);
    }

private:
    template <std::size_t... Indexes, typename... ExtraArgs>
    auto call(std::index_sequence<Indexes...>, ExtraArgs&&... extra_args) {
        return (m_obj->*m_func)(m_args.template get<Indexes>()...,
                                std::forward<ExtraArgs>(extra_args)...);
    }
